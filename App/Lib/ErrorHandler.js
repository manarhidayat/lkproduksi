  ...
  switch (payload.problem) {
    case 'CLIENT_ERROR':
      ...
      if (Platform.OS === 'android')
        return ToastAndroid.show(`Sorry something error`, ToastAndroid.LONG);
      console.tron.warn(payload);
    ...
    case 'TIMEOUT_ERROR':
      if (Platform.OS === 'android')
        return ToastAndroid.show(`Server didn't respond`, ToastAndroid.SHORT);
      console.tron.warn(payload);
      return console.tron.warn(`Server didn't respond`);
    case 'CONNECTION_ERROR':
      if (Platform.OS === 'android')
        return ToastAndroid.show('Server not available', ToastAndroid.SHORT);
      console.tron.warn(payload);
      return console.tron.warn(`Server not available`);
    case 'NETWORK_ERROR':
      if (Platform.OS === 'android')
        return ToastAndroid.show('Network not available', ToastAndroid.SHORT);
      console.tron.warn(payload);
...
function SuccessAlert(msg) {
  return setTimeout(() => {
    Alert.alert('Selamat!', msg);
  }, 25);
}

export {ErrorHandler, ErrorAlert, SuccessAlert};
